...
export default function Login() {
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      // Extract the WebContainer URL if we're in that environment
      const webContainerUrl = currentUrl.includes('webcontainer') 
        ? currentUrl.split('/').slice(0, 3).join('/') 
        : baseUrl;
      
      console.log("Using redirect URL:", `${webContainerUrl}/dashboard`);
      
      const { data, error } = await supabase.auth.signInWithOAuth({
        provider: 'google',
        options: {
          redirectTo: `${webContainerUrl}/dashboard`,
          queryParams: {
            access_type: 'offline',
            prompt: 'consent',
          },
